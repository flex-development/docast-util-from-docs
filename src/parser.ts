...
class Parser extends Location {
  ...
  protected get blockTag(): P<kinds, BlockTag> {
    return apply(seq(
       ...
       * Block tag name.
       *
       * @const {BlockTag['name']} name
       */
      const name: BlockTag['name'] = tag.text.slice(1)
        ...
        children: this.children<BlockTag>([
          ...
          ...this.applyMarkdown(markdown, markdown && codeblocks.some(check => {
            return isString(check)
              ? includes([name, tag.text], check)
              : [name, tag.text].some(str => check.test(str))
          }) && !markdown.text.startsWith('```'))
        ]),
        name,
        position: {
  ...
  protected get comment(): P<kinds, Comment> {
    return apply(seq(
      ...
      return u(types.comment, {
        children: sift([description, ...blockTags]),
        code: null,
   ...
   * @return {Exclude<Content, DocastNode>[]} mdast child node array
   */
      ...
      mdastExtensions: [
        {
          ...
          exit: {
            ...
            [types.inlineTag](this: CompileContext, token: MToken): void {
              ...
              ok(match.groups.name, 'expected `match.groups.name`')
              ok(match.groups.value, 'expected `match.groups.value`')

              node.name = match.groups.name
              node.tag = match.groups.tag
